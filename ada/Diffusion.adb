...
procedure Diffusion is
    ...
    distance_between_blocks : float := room_dimension / float(room_dimension);
    timestep : float := room_dimension / speed_of_gas_molecules / float(room_dimension);
    DTerm : float := diffusion_coefficent * timestep / float (distance_between_blocks*distance_between_blocks);
    pass : Integer :=0;
    time : float := 0.0;
    ratio : float := 0.0;
